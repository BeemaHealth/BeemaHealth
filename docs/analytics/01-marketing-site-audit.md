...
## 2026-08-09 live remediation evidence

A clean headless-Chrome capture on `beemahealth.com` confirmed separate direct
`gtag.js` requests for `AW-18301765593` and `G-03PMCCSD3R`, in addition to the
GTM container. The direct Ads shell snippet was removed. GA4 and Ads now share
the single loader in `ad-conversions.ts`; GTM remains for Conversion Linker,
`intake_handoff`, and the Bask cross-domain flow. No event parameters changed.

The site does deliver a meta CSP. The same clean capture reached
`googleads.g.doubleclick.net` and `www.google.com`, so those required measurement
origins (plus the associated Google analytics endpoints) are now narrowly
allowed in `connect-src`. The recipe browser audit continues to verify that no
serving, multiplier, category, symptom, or nutrition values are sent.
