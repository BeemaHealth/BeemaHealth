  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {showToc && sections.length > 3 && (
        <nav aria-label="Table of contents" className="mt-8 rounded-2xl border border-border bg-background px-5 py-4">
      ...
      <div className="mt-10 space-y-10">
        {sections.map((section) => (
          <section key={section.id} id={section.id} className="scroll-mt-24">
            ...
            <div className={cn("mt-3 space-y-4 text-sm leading-relaxed text-muted-foreground", "legal-prose")}>
              {section.content}
            </div>
          </section>
        ))}
      ...
      <p className="mt-12 border-t border-border pt-8 text-xs leading-relaxed text-muted-foreground">
        ...
        , and{" "}
        <Link to="/legal/telehealth-consent" className="text-foreground underline-offset-2 hover:underline">
          Telehealth Consent
        </Link>
        .
      </p>
