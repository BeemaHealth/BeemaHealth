...
export const Route = createFileRoute("/dashboard")({
  beforeLoad: () => {
    if (!getSession()) throw redirect({ to: "/login", search: { redirect: "/dashboard" } });
  },
...
function DashboardPage() {
  ...
  const status = review?.status ?? intake?.status ?? "draft";
      ...
      <div className="veya-container py-12 md:py-16">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          <SurfaceCard className="p-6">
            ...
            <p className="mt-3 text-2xl font-bold text-primary">{STATUS_LABELS[status] ?? status}</p>
            {intake?.submitted_at && (
              <p className="mt-2 text-sm text-muted-foreground">
                Submitted {new Date(intake.submitted_at).toLocaleDateString()}
              </p>
            )}
            {eligibility?.treatment_interest && (
              <p className="mt-2 text-sm text-muted-foreground">
                Treatment interest: {eligibility.treatment_interest.replace(/_/g, " ")}
              </p>
            )}
          </SurfaceCard>
          ...
          <SurfaceCard className="p-6">
            ...
            {review?.patient_note ? (
              <p className="mt-3 text-sm leading-relaxed text-foreground">{review.patient_note}</p>
            ) : (
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Button asChild>
            <Link to="/intake">{status === "draft" ? "Continue intake" : "Update intake"}</Link>
          </Button>
